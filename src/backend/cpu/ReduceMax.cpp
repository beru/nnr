...
struct ReduceMax_operator : reduce_base_t {
    template <typename T>
    bool exec() {
        ...
        if (x->ndata == 0) {
            if constexpr (std::is_floating_point_v<T>) {
                ...
                for (size_t i = 0; i < outputs[0]->ndata; ++i) py[i] = std::numeric_limits<T>::lowest();
            }
            return true;
        }
...
// @nnr-meta-op mt=no
operator_t* resolver_default_op_ReduceMax(int opset, pool_t& pool) { return pool_new<ReduceMax_operator>(pool); }
