...
struct CumSum_operator : public operator_t {
    int exclusive;
    int reverse;
    ...
    bool exec() override {
        return typed_exec<CumSum_operator,
...
// @nnr-meta-op mt=no
operator_t* resolver_default_op_CumSum(int opset, pool_t& pool)
{
    return pool_new<CumSum_operator>(pool);
}
